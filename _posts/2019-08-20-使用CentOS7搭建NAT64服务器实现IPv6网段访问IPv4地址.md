...
description: 
author: default
date: 2019-08-20 18:16:24 +0800
categories: [Linux, 服务的安装部署]
tags: [NAT64, DNS64, jool]
pin: false
math: false
mermaid: false
toc: true
...
编辑 `/etc/selinux/config`{: .filepath} ，将 `SELINUX` 的参数改为 `disabled`。
...
将 iso 镜像挂载到 `/media/cdrom`{: .filepath} 目录下
...
将 `/etc/yum.repos.d/`{: .filepath} 目录下的 repo 文件恢复
...
编辑 `/etc/sysconfig/modules/jool.modules`{: .filepath} 文件，如果没有就创建一个。
...
```
{: file='/etc/sysconfig/modules/jool.modules'}
...
编辑 named 服务配置文件 `/etc/named.conf`{: .filepath}
