...
::: details **CentOS 7 查询空密码且可以登录的用户**

```shell
awk -F ":" '($NF=="/bin/bash"){print $1}' /etc/passwd | xargs -I {} awk -F ":" '($2=="!!")&&($1=="{}"){print $1}' /etc/shadow
```

:::

::: details **CentOS 7 查找指定时间区间内多个不同目录下创建的文件**

```shell
find / -depth -path "/etc/*" -o -path "/tmp/*" -newermt "2025-07-28 16:02:00" ! -newermt "2025-07-28 16:04:00"
```
