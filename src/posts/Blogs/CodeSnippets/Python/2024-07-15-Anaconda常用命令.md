...
conda 有时候安装软件会非常慢。设置国内镜像的话可以使安装更快捷一些。
...
卸载 `package_name` 这个包的同时，将依赖于这个包的所有其它包也同时卸载。

```shell
conda uninstall package_name
```

如果不想删除依赖包，只想卸载 package_name 包，可以使用这个命令。
...
删除所有的安装包及 cache (索引缓存、锁定文件、未使用过的包和 tar 包)。
